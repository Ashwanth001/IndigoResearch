"""
Retrain GNN-11F+LLM (FP32, gradient checkpointing, capability edges from FinLang).

Fixes applied vs the previous training run:
- Removed FP16/GradScaler (was causing gradient instability: clip_grad_norm_ was
  operating on scaled gradients, effectively doing nothing)
- Added from_dev() to release GPU memory after each training step
- Removed torch.cuda.empty_cache() calls from inside the hot loop

Run this script from the project root:
    python _run_step12_llm.py
"""

import os, sys, pickle, gc, warnings
import numpy as np
import pandas as pd
import torch
...
print(f'Checkpoint target: {CKPT_LLM}')

if os.path.exists(CKPT_LLM):
    print('Checkpoint already exists — delete to re-train.')
    sys.exit(0)
...
def build_sample(obs_yr, ldf):
    snaps = [build_snap_llm(y) for y in range(obs_yr - 4, obs_yr + 1)]
...
tr = [build_sample(y, train_lbl) for y in sorted(train_lbl['year'].unique())]
va = build_sample(VAL_YEAR, val_lbl)
print(f'Training years: {[s["labels"]["edge_label_index"].shape[1] for s in tr]}')

# ── Train ─────────────────────────────────────────────────────────────────────
meta = tr[0]['snapshots'][0].metadata()
enc  = BipartiteEncoder(11, HIDDEN, meta).to(DEVICE)
...
print(f'Training GNN-11F+LLM  |  c_in=11  |  pos_weight={n_neg/n_pos:.1f}x  |  Grad-ckpt: ON  |  FP32')
...
for ep in range(1, EPOCHS + 1):
    ...
    if ep % 5 == 0 or no_imp >= PATIENCE:
        print(f'  Ep {ep:3d}  loss={ep_loss/len(tr):.4f}  val_PR-AUC={vpa:.4f}  best={best_vpa:.4f}  no_imp={no_imp}')
...
torch.save({'mdl_state':  {k: v.cpu() for k, v in mdl.state_dict().items()},
            ...
            'c_in': 11, 'hidden': HIDDEN, 'meta': meta}, CKPT_LLM)
print(f'Checkpoint -> {CKPT_LLM}')
print('Done.')
